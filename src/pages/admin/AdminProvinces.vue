<template>
  <div>
    ...
    <q-table
      ...
      :no-data-label="countryId ? t('common.noData') : t('admin.selectCountry')"
...
import {
  ...
  apiAdminCreateProvince,
  apiAdminUpdateProvince,
  apiAdminDeleteProvince,
} from 'src/api';
...
const columns = computed<QTableColumn[]>(() => [
  ...
  { name: 'state_code', label: t('admin.code'), field: 'state_code', align: 'left' },
...
function openCreate() {
  // Filtrda tanlangan mamlakat ma'lumotlari avtomatik to'ldiriladi
  const c = selectedCountry.value;
  Object.assign(form, {
    _id: '', id: null, name: '',
    country_id: c?.value ?? null, country_code: c?.iso2 ?? '', country_name: c?.name ?? '',
    state_code: '', translations: { uz: '', ru: '', en: '' },
  });
  error.value = '';
  dialog.value = true;
}
...
async function save() {
  ...
  try {
    ...
    if (form._id) {
      ...
      const res = await apiAdminCreateProvince({ ...payload, id: form.id });
...
async function executeDelete() {
  ...
  saving.value = true;
  try {
    await apiAdminDeleteProvince(target._id);
    provinces.value = provinces.value.filter((p) => p._id !== target._id);
